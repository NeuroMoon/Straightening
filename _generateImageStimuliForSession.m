...
img_diam_pixels         = round(ImageSequences.CONSTANTS.NATIVE_IMAGE_DIAM_DEG * deg2pix);
...
apply_mask          = ImageSequences.CONSTANTS.APPLY_VIGNETTE || img_diam_pixels ~= vignette_diam_pixels;
if(apply_mask)
    % Vignette (need to resize when generating actual image stimulus)
    flattop8                = im2double(imread(fullfile(ImageSequences.CONSTANTS.STIMULUS_FOLDER, ImageSequences.CONSTANTS.MASK_FOLDER, 'Flattop8.tif')));
...
for i = 1:p.trial.(modName).NUM_UNIQ_IMAGES
    ...
    % X2 subsampling
    img_filepath        = fullfile(ImageSequences.CONSTANTS.STIMULUS_FOLDER, ImageSequences.CONSTANTS.SUBSAMPLE_FOLDER, sprintf('%03d.png', p.trial.(modName).UNIQ_IMAGES(i)));
